...
interface StoredSharedConnection {
  configured: boolean
  apiUrl: string | null
...
function apiClientFor(cfg: McpConfig, apiUrl: string, token?: string): ApiClient {
...
function requireTokenForResolvedRuntime(runtime: Runtime, token?: string): void {
  if (runtime.deploymentMode === 'local') return
  requireTokenForServerMode({
    PM_USER_TOKEN: token,
...
async function loadStoredSharedConnection(personalApi: ApiClient): Promise<{ apiUrl: string; token: string } | null> {
  try {
    const status = await personalApi.get<StoredSharedConnection>('/dashboard/shared-connection', { includeToken: true })
    if (!status.configured || !status.apiUrl || !status.token) return null
...
async function resolveSharedContext(
...
): Promise<{ api: ApiClient; runtime: Runtime } | null> {
  ...
  if (!sharedApiUrl || !sharedToken) return null
...
export async function resolveStartupContext(cfg: McpConfig): Promise<StartupContext> {
