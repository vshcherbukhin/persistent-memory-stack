...
export function compareUpgradeVersions(a, b) {
...
export function satisfiesUpgradeRange(version, range) {
...
function rangePermitsVersionNewerThan(range, target) {
    const candidates = new Set([nextPatchVersion(target)]);
    ...
    return [...candidates].some((candidate) => (compareUpgradeVersions(candidate, target) > 0
        && satisfiesUpgradeRange(candidate, range)));
}
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function rejectUnexpectedFields(value, name, allowed) {
    const unexpected = Object.keys(value).find((key) => !allowed.includes(key));
    if (unexpected)
        throw new Error(`${name} contains unsupported field ${unexpected}.`);
}
...
export function validateUpgradeContract(value, options) {
    if (!isRecord(value))
        throw new Error('Release upgrade contract must be an object.');
    ...
    if (value.schemaVersion !== 1)
        throw new Error('Release upgrade contract must use schemaVersion 1.');
    ...
    if (!options.availableReleases.has(release))
        throw new Error(`Release ${release} is unavailable from the trusted branch.`);
    ...
    const bridges = value.bridges.map((raw, index) => {
        if (!isRecord(raw))
            throw new Error(`bridges[${index}] must be an object.`);
        rejectUnexpectedFields(raw, `bridges[${index}]`, ['from', 'to', 'requires', 'reason']);
        const to = requireVersion(raw.to, `bridges[${index}].to`);
        if (to !== release)
            throw new Error(`Bridge ${index} must target release ${release}.`);
        if (!options.availableReleases.has(to))
            throw new Error(`Bridge target ${to} is unavailable from the trusted branch.`);
        return {
    ...
    const requiredStops = value.requiredStops.map((raw, index) => {
        if (!isRecord(raw))
            throw new Error(`requiredStops[${index}] must be an object.`);
        ...
        if (!options.availableReleases.has(stopRelease))
            throw new Error(`Required stop ${stopRelease} is unavailable from the trusted branch.`);
        if (seenStops.has(stopRelease))
            throw new Error(`Required stop ${stopRelease} is duplicated.`);
...
export function planUpgradePath(sourceVersion, target, contracts) {
    if (!contracts.has(target.release))
        throw new Error(`Target contract is unavailable: ${target.release}`);
    ...
    for (const stop of target.requiredStops) {
        if (!satisfiesUpgradeRange(current, stop.when))
            continue;
        const stopContract = contracts.get(stop.release);
        if (!stopContract)
            throw new Error(`Required stop contract is unavailable: ${stop.release}`);
    ...
    if (sourceMajor !== targetMajor) {
        const bridge = target.bridges.find((candidate) => (candidate.to === target.release
            && satisfiesUpgradeRange(current, candidate.from)
            && satisfiesUpgradeRange(current, candidate.requires)));
        if (!bridge)
            throw new Error(`A declared bridge is required from ${current} to ${target.release}.`);
    }
