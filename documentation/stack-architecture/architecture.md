...
| `apps/update-runner/` | `persistent-memory-update-runner` | Restricted internal sidecar for published-release discovery and update status; execution stays in the terminal coordinator. | `../apps/update-runner/README.md` |
...
- **`update-runner`** (`:9092`, internal-only) — the separate update metadata sidecar. It is token-gated by `UPDATE_RUNNER_TOKEN`, automatically checks published stable GitHub Releases and their exact tagged source for full-local dashboard update cards without source credentials, and exposes update status/logs. Its legacy start route rejects execution and directs operators to the terminal updater, whose coordinator owns compatibility planning, snapshots and lifecycle changes. It is deliberately not part of `docker-control`.
